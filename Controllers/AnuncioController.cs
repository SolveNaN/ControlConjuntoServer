...
namespace ControlConjuntoServer.Controller
{
    ...
    public class AnuncioController : ControllerBase
    {
        ...
        public async Task<ActionResult<string>> GetConexionDB()
        {
            try
            {
                var usuarios = await _context.Anuncios.ToListAsync();
        ...
        {
            var lista = await _context.Anuncios.ToListAsync();
            return Ok(lista);
        }
        ...
        [HttpGet]
        [Route("ConsutarId/{id}")]
        public async Task<ActionResult<List<Anuncio>>> GetSingleAnuncio(int id)
        {
            var miobjeto = await _context.Anuncios.FirstOrDefaultAsync(ob => ob.Id == id);
            if (miobjeto == null)
            {
                return NotFound(" :/");
            }
        ...
        [HttpPost("Crear")]
        public async Task<ActionResult<string>> CreateAnuncio(Anuncio objeto)
        {
            try
            {
                _context.Anuncios.Add(objeto);
        ...
        public async Task<ActionResult<List<Anuncio>>> UpdateAnuncio(Anuncio objeto)
        {

            var DbObjeto = await _context.Anuncios.FindAsync(objeto.Id);
            if (DbObjeto == null)
        ...
        [HttpDelete]
        [Route("Eliminar/{id}")]
        public async Task<ActionResult<string>> DeleteAnuncio(int id)
        {
            var DbObjeto = await _context.Anuncios.FirstOrDefaultAsync(Ob => Ob.Id == id);
            if (DbObjeto == null)
            {
            ...
            try
            {
                _context.Anuncios.Remove(DbObjeto);
                await _context.SaveChangesAsync();
