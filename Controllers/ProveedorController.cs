...
namespace ControlConjuntoServer.Controller
{
    ...
    public class ProveedorController : ControllerBase
    {
        ...
        public async Task<ActionResult<string>> GetConexionDB()
        {
            try
            {
                var usuarios = await _context.Proveedors.ToListAsync();
        ...
        [HttpGet]
        [Route("ConsutarId/{id}")]
        public async Task<ActionResult<List<Proveedor>>> GetSingleProveedor(int id)
        {
            var miobjeto = await _context.Proveedors.FirstOrDefaultAsync(ob => ob.Id == id);
            if (miobjeto == null)
            {
                return NotFound(" :/");
            }
        ...
        [HttpPost("Crear")]
        public async Task<ActionResult<string>> CreateProveedor(Proveedor objeto)
        {
            try
            {
        ...
        public async Task<ActionResult<List<Proveedor>>> UpdateProveedor(Proveedor objeto)
        {

            var DbObjeto = await _context.Proveedors.FindAsync(objeto.Id);
            if (DbObjeto == null)
        ...
        [HttpDelete]
        [Route("Eliminar/{id}")]
        public async Task<ActionResult<string>> DeleteProveedor(int id)
        {
            var DbObjeto = await _context.Proveedors.FirstOrDefaultAsync(Ob => Ob.Id == id);
            if (DbObjeto == null)
            {
