...
namespace ControlConjuntoServer.Controller
{
    ...
    public class ReservaController : ControllerBase
    {
        ...
        public async Task<ActionResult<string>> GetConexionDB()
        {
            try
            {
                var usuarios = await _context.Reservas.ToListAsync();
        ...
        [HttpGet]
        [Route("ConsutarId/{id}")]
        public async Task<ActionResult<List<Reserva>>> GetSingleReserva(int id)
        {
            var miobjeto = await _context.Reservas.FirstOrDefaultAsync(ob => ob.Id == id);
            if (miobjeto == null)
            {
                return NotFound(" :/");
            }
        ...
        [HttpPost("Crear")]
        public async Task<ActionResult<string>> CreateReserva(Reserva objeto)
        {
            try
            {
                _context.Reservas.Add(objeto);
        ...
        public async Task<ActionResult<List<Reserva>>> UpdateReserva(Reserva objeto)
        {

            var DbObjeto = await _context.Reservas.FindAsync(objeto.Id);
            if (DbObjeto == null)
        ...
        [HttpDelete]
        [Route("Eliminar/{id}")]
        public async Task<ActionResult<string>> DeleteReserva(int id)
        {
            var DbObjeto = await _context.Reservas.FirstOrDefaultAsync(Ob => Ob.Id == id);
            if (DbObjeto == null)
            {
            ...
            try
            {
                _context.Reservas.Remove(DbObjeto);
                await _context.SaveChangesAsync();
