...
namespace ControlConjuntoServer.Controller
{
    ...
    public class ServicioController : ControllerBase
    {
        ...
        public async Task<ActionResult<string>> GetConexionDB()
        {
            try
            {
                var usuarios = await _context.Servicios.ToListAsync();
        ...
        [HttpGet]
        [Route("ConsutarId/{id}")]
        public async Task<ActionResult<List<Servicio>>> GetSingleServicio(int id)
        {
            var miobjeto = await _context.Servicios.FirstOrDefaultAsync(ob => ob.Id == id);
            if (miobjeto == null)
            {
                return NotFound(" :/");
            }
        ...
        [HttpPost("Crear")]
        public async Task<ActionResult<string>> CreateServicio(Servicio objeto)
        {
            try
            {
                _context.Servicios.Add(objeto);
        ...
        public async Task<ActionResult<List<Servicio>>> UpdateServicio(Servicio objeto)
        {

            var DbObjeto = await _context.Servicios.FindAsync(objeto.Id);
            if (DbObjeto == null)
        ...
        [HttpDelete]
        [Route("Eliminar/{id}")]
        public async Task<ActionResult<string>> DeleteServicio(int id)
        {
            var DbObjeto = await _context.Servicios.FirstOrDefaultAsync(Ob => Ob.Id == id);
            if (DbObjeto == null)
            {
            ...
            try
            {
                _context.Servicios.Remove(DbObjeto);
                await _context.SaveChangesAsync();
