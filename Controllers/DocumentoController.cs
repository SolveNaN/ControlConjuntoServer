...
namespace ControlConjuntoServer.Controller
{
    ...
    public class DocumentoController : ControllerBase
    {
        ...
        public async Task<ActionResult<string>> GetConexionDB()
        {
            try
            {
                var usuarios = await _context.Documentos.ToListAsync();
        ...
        {
            var lista = await _context.Documentos.ToListAsync();
            return Ok(lista);
        }
        ...
        [HttpGet]
        [Route("ConsutarId/{id}")]
        public async Task<ActionResult<List<Documento>>> GetSingleDocumento(int id)
        {
            var miobjeto = await _context.Documentos.FirstOrDefaultAsync(ob => ob.Id == id);
            if (miobjeto == null)
            {
                return NotFound(" :/");
            }
        ...
        [HttpPost("Crear")]
        public async Task<ActionResult<string>> CreateDocumento(Documento objeto)
        {
            try
            {
                _context.Documentos.Add(objeto);
        ...
        public async Task<ActionResult<List<Documento>>> UpdateDocumento(Documento objeto)
        {

            var DbObjeto = await _context.Documentos.FindAsync(objeto.Id);
            if (DbObjeto == null)
        ...
        [HttpDelete]
        [Route("Eliminar/{id}")]
        public async Task<ActionResult<string>> DeleteDocumento(int id)
        {
            var DbObjeto = await _context.Documentos.FirstOrDefaultAsync(Ob => Ob.Id == id);
            if (DbObjeto == null)
            {
            ...
            try
            {
                _context.Documentos.Remove(DbObjeto);
                await _context.SaveChangesAsync();
