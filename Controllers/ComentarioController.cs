...
namespace ControlConjuntoServer.Controller
{
    ...
    public class ComentarioController : ControllerBase
    {
        ...
        public async Task<ActionResult<string>> GetConexionDB()
        {
            try
            {
                var usuarios = await _context.Comentarios.ToListAsync();
        ...
        {
            var lista = await _context.Comentarios.ToListAsync();
            return Ok(lista);
        }
        ...
        [HttpGet]
        [Route("ConsutarId/{id}")]
        public async Task<ActionResult<List<Comentario>>> GetSingleComentario(int id)
        {
            var miobjeto = await _context.Comentarios.FirstOrDefaultAsync(ob => ob.Id == id);
            if (miobjeto == null)
            {
                return NotFound(" :/");
            }
        ...
        [HttpPost("Crear")]
        public async Task<ActionResult<string>> CreateComentario(Comentario objeto)
        {
            try
            {
                _context.Comentarios.Add(objeto);
        ...
        public async Task<ActionResult<List<Comentario>>> UpdateComentario(Comentario objeto)
        {

            var DbObjeto = await _context.Comentarios.FindAsync(objeto.Id);
            if (DbObjeto == null)
        ...
        [HttpDelete]
        [Route("Eliminar/{id}")]
        public async Task<ActionResult<string>> DeleteComentario(int id)
        {
            var DbObjeto = await _context.Comentarios.FirstOrDefaultAsync(Ob => Ob.Id == id);
            if (DbObjeto == null)
            {
            ...
            try
            {
                _context.Comentarios.Remove(DbObjeto);
                await _context.SaveChangesAsync();
